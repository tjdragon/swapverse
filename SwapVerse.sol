 ...
 * Lots of bugs I guess as well (need to handle deletes better, add more tests, HTLC, Overflows, etc)
...
struct Trade {
    address counter_party_1;
    uint256 amount_1;
    address token_1;
    address counter_party_2;
    uint256 amount_2;
    address token_2;
}
...
contract SwapVerse {
    ...
    event SettlementInstructionEvent(address from_participant, address to_participant, address token, uint256 amount);
    ...
    function get_order_tuple(uint256 id) public view returns (address, uint256, bool, uint256, uint256) {
        Order storage lo = _orders[id];
        return (lo.owner, lo.id, lo.is_buy, lo.price, lo.size);
    }

    function get_trade_size() public view returns (uint256) {
        return _trades.length;
    }

    function get_trade(uint256 index) public view returns (address, uint256, address, address, uint256, address) {
        require(index < _trades.length);
        Trade storage lt = _trades[index];
        return (lt.counter_party_1, lt.amount_1, lt.token_1, lt.counter_party_2, lt.amount_2, lt.token_2);
    }
    ...
    function process_matched_order(uint256 orderId, uint256[] storage matched_ids, bool is_buy) private {
        ...
        for(uint i = 0; i < matched_ids.length && size_left > 0; i++) {
            ...
            if (size_left >= matched_order.size) { // Order size is >= current matched order
                ...
                _trades.push(Trade(order.owner, matched_order.size, _token_address_1, matched_order.owner, matched_order.size*order.price, _token_address_2));
                process_trade(matched_order.owner, order.owner,  matched_order.size, order.price, is_buy);
                ...
                emit TradeEvent(leg1, leg2);
                process_trade(matched_order.owner, order.owner,  matched_order.size, order.price, is_buy);
                _trades.push(Trade(order.owner, matched_order.size*order.price, _token_address_1, matched_order.owner,matched_order.size, _token_address_2));
                remove_order(matched_order.id);
                size_left = 0;
    ...
    function process_trade(address buyer, address seller, uint256 size, uint256 price, bool is_buy) private {
        // Need to worry about overflow etc -- also implement HTLC for atomic swap

        if (is_buy) {
            uint256 amount_to_transfer = size * price;
            emit SettlementInstructionEvent(seller, buyer, _token_address_1, size);
            emit SettlementInstructionEvent(buyer, seller, _token_address_2, amount_to_transfer);
            _trades.push(Trade(seller, size, _token_address_1, buyer, amount_to_transfer, _token_address_2));
            // console.log("_token_1.allowance %s", _token_1.allowance(seller, address(this)));
            // console.log("_token_1.allowance %s", _token_1.allowance(buyer, address(this)));

            require(_token_1.allowance(seller, address(this)) >= size, "{Buy} Not enough funds for settlement");
            require(_token_1.allowance(buyer, address(this)) >= amount_to_transfer, "{Buy} Not enough funds for settlement");
            bool sent = _token_1.transferFrom(seller, buyer, size);
            require(sent, "Transfer 1 failed");
            sent = _token_2.transferFrom(buyer, seller, amount_to_transfer);
            require(sent, "Transfer 2 failed");
        } else {
            uint256 amount_to_transfer = size * price;
            emit SettlementInstructionEvent(seller, buyer, _token_address_2, amount_to_transfer);
            emit SettlementInstructionEvent(buyer, seller, _token_address_1, size);
            _trades.push(Trade(seller, amount_to_transfer, _token_address_2, buyer, size, _token_address_1));
            // console.log("_token_2.allowance %s", _token_2.allowance(seller, address(this)));
            // console.log("_token_2.allowance %s", _token_2.allowance(buyer, address(this)));

            require(_token_2.allowance(seller, address(this)) >= amount_to_transfer, "{Sell} Not enough funds for settlement");
            require(_token_2.allowance(buyer, address(this)) >= size, "{Sell} Not enough funds for settlement");
           
            bool sent = _token_2.transferFrom(seller, buyer, amount_to_transfer);
            require(sent, "Transfer 1 failed");
            sent = _token_1.transferFrom(buyer, seller, size);
            require(sent, "Transfer 2 failed");
        }
    }
}